	...
	if ($Result['result'] == 'success') {
		if (isset($Result['data']->user_info->auth)) {
			if ($Result['data']->user_info->auth != 0) {
				...
				$returnData = array('result' => 'error', 'message' => 'Informaçoes Inválidas');
			}
		}
		else {
			$returnData = array('result' => 'error', 'message' => 'Informaçoes Inválidas');
...
if (isset($_POST['action']) && ($_POST['action'] == 'getMoviesDataFromCategoryId')) {
	...
	if ($ApiData['result'] == 'success') {
		...
		if ($datalimit < $counter) {
			...
			echo '">Carregar Mais Resultados <i class="LoadingMoreFa fa fa-spin fa-spinner hideOnload"></i></button>' . "\r\n\t\t\t" . '</center>' . "\r\n\t\t\t";
		}
		...
		if (isset($SeriesData['data']->episodes) && !empty($SeriesData['data']->episodes)) {
			...
			if (!empty($SeriesData['data']->episodes)) {
				...
				foreach ($SeriesData['data']->episodes as $episodes) {
					foreach ($episodes as $episodesData) {
						$SeasonsIdData[$episodesData->season] = 'Temporada';
					}
				}
			}
			...
			foreach ($Appepisodes as $episodes) {
				foreach ($episodes as $episodesData) {
					...
					echo '" data-episid="';
					echo $episodesData->id;
					echo '" class="backToInfo btn btn-info hideOnLoad rippler rippler-default">Voltar</button>' . "\r\n" . '  ' . "\t\t\t\t\t\t\t\t\t" . '<div id="epis-';
					echo $episodesData->id;
					echo '" class="tab-pane fade in ';
					...
					echo '\')">Assista Agora</button>' . "\r\n\t\t\t\t\t" . '                  </div>' . "\r\n\t\t\t\t\t" . '                </div>' . "\r\n" . '      ' . "\t\t\t\t\t\t\t\t";
					++$CounterCon3;
				}
			}

			echo "\t\t" . '              ' . "\t\t\r\n\t\t" . '              </div>' . "\r\n\t\t" . '              <!--tab-content--> ' . "\r\n\t\t" . '            </div>' . "\r\n\t\t" . '            <div class="clearfix"></div>' . "\r\n\t\t" . '            <h3 class="descHeading">Descrição</h3>' . "\r\n\t\t" . '            <p class="decription">';
			echo $MainMovieDesc;
			echo '</p>' . "\r\n\t\t" . '          </div>' . "\r\n\t\t" . '        </div>' . "\r\n\t\t" . '      </div>' . "\r\n\t\t" . '    </div>' . "\t\r\n\t\t" . '     ';
			exit();
		}
...
if (isset($_POST['action']) && ($_POST['action'] == 'getMovieInfo')) {
	...
	if ($movieData['result'] == 'success') {
		...
		echo $movieData['data']->info->releasedate != '' ? date("d/m/Y", strtotime($movieData['data']->info->releasedate)) : 'n/A';
		echo '</li>' . "\r\n" . '                <li class="i-duration">';
		echo $movieData['data']->info->duration_secs != '' ? round(intval($movieData['data']->info->duration_secs) / 60) . ' Minutos' : 'n/A';
		echo ' </li>' . "\r\n" . '                <li class="i-movie">';
		...
		if (40 <= strlen($movieData['data']->info->cast)) {
			echo $movieData['data']->info->cast . '</p><button class="showCast btn btn-sm btn-info rippler rippler-default">Exibir Elenco</button>';
		}
		else {
		...
		else if (isset($movieData['data']->info->plot) && ($movieData['data']->info->plot != '')) {
		...
		echo '.';
		echo $Mainextension;
		echo '\')" class=" rippler rippler-default">Assista Agora</button>' . "\r\n" . '              </div>' . "\r\n" . '            </div>' . "\r\n" . '          </div>' . "\r\n" . '        </div>' . "\r\n" . '      </div>' . "\r\n" . '    </div>' . "\r\n\t\t";
	}
}
