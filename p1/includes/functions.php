...
function webtvpanel_CheckstreamLine($username = '', $password = '', $hostURL = '')
{
	...
	if (!empty($CallApi) && ($CallApi['result'] == 'success')) {
		if (isset($CallApi['data']->user_info->auth)) {
			if ($CallApi['data']->user_info->auth != 0) {
				if ($CallApi['data']->user_info->status == 'Active') {
					$returnData = '1';
				}
			}
		}
...
function webtvpanel_parentcondition($Text = '')
{
	...
	if ($parentenable == 'on') {
		if ((webtvpanel_like_match('%adults%', $Text) == 1) || (webtvpanel_like_match('%adult%', $Text) == 1) || (webtvpanel_like_match('%Adults%', $Text) == 1) || (webtvpanel_like_match('%XXX%', $Text) == 1) || (webtvpanel_like_match('%Porn%', $Text) == 1) || (webtvpanel_like_match('%xxx%', $Text) == 1) || (webtvpanel_like_match('%Sexy%', $Text) == 1) || (webtvpanel_like_match('%foradults%', $Text) == 1) || (webtvpanel_like_match('%ADULTE%', $Text) == 1) || (webtvpanel_like_match('%adulte%', $Text) == 1)) {
			$returnData = 1;
		}
	}
